...
- [ ] **연장 불가 모드(기간 강력 잠금)** (`Feat/AbsoluteLock` 구현 완료 2026-07-13 —
  ...
  - [ ] 시뮬 육안(런북 하단 체크리스트): 시트 2단계·연장 칩 활성/비활성·배지·차단 alert·
        LockOptions 안내 카드·설정 행 24pt 인포 버튼+제목 없는 2문단 팝오버·토글 상태와 무관하게
        항상 AccentColor 회전·발광 테두리+활성 순간 1회 전체 섬광(0.14초 점등→
        1초 유지→1.2초 복귀) 후 상시 효과로 복귀
        (잘림·토글 터치 간섭 없음, Reduce Motion은 회전 없이 밝기 변화만)·
        다크 모드·큰 글자·en/ja
  - [ ] **UX 다듬기 8건 재검증(2026-07-29 변경 — 시뮬로 완료 처리 금지인 항목은 소리·햅틱뿐)**
    ...
    - [ ] 연장 불가 행 제목이 적용 전 "연장 불가 기간 설정"(2026-08-15 문구 변경) → 적용 중
          "연장 불가 중", en/ja(`Set Locked-In Period`/`延長不可期間を設定`)도 잘리지 않는지
