...
- **금고 모드 진입은 그룹 카드의 전용 행으로 유지합니다**(2026-07-13 재검토·유지 결정). 규칙 편집기(`RuleEditorSheet`) 안에 요일별 규칙 토글처럼 통합하자는 안을 검토했으나 기각했습니다: ① 금고를 켜면 **규칙 편집기 진입 자체가 차단**되므로 자물쇠를 잠긴 방 안에 두는 꼴이 됩니다 — 남은 일수 확인과 **기간 연장**(허용된 유일한 조작) 진입로가 사라집니다. 살리려면 "진입은 허용하되 규칙 섹션만 잠금"이라는 부분 잠금이 필요해 단순 방어(진입 차단 3곳)가 깨지고 우회 표면이 늘어납니다. ② 편집기 완료는 광고 게이트를 타는데, 되돌릴 수 없는 금고 활성화가 규칙 변경에 묻어 한 번에 커밋되는 구조는 위험합니다. ③ 요일별 규칙은 "규칙의 형태", 금고는 "규칙을 못 바꾸게 잠그는 약정"이라 개념 층위가 다릅니다. 카드 우상단 `⋯` 메뉴로 흡수하는 안(카드가 짧아짐)도 검토했으나, **발견성이 이 기능의 승패를 가른다**고 보아 카드 한 줄의 비용을 수용했습니다.
