...
- 금고 모드(기간 약정 강력 잠금, 2026-07-12 기획)는 opt-in **자기 약정**입니다 — 기본 경험은 여전히 비용감(통행료)이고, 사용자가 스스로 선택한 기간(최대 14일, 자정 경계 만료)만 API가 허용하는 최대 봉쇄(연장·편집·삭제 차단 + `denyAppRemoval` + 자동 날짜 강제)를 제공합니다. "완전한 차단 우회 방지 보장 안 함"은 유지됩니다: Screen Time 권한 철회는 어떤 앱도 API로 못 막으며, 감지·솔직 고지로 대응합니다. 스크린타임 암호 안내 방식(랜덤 코드 생성 → 사용자가 설정에 입력 → 앱이 보관, Password Locker 패턴)은 검토 후 사용자 결정으로 제외했습니다.
