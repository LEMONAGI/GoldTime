...
enum AnalyticsEvent {
    ...
    case groupSnapshot(appliedGroupCount: Int)
    ...
    case ruleGroupSnapshot(payload: RuleGroupSnapshotAnalytics)
    ...
    case strictLockCompleted(days: Int)
    ...
    var name: String {
        ...
        case .ruleGroupSnapshot(let payload): return payload.eventName
    ...
    var parameters: [String: Any] {
        switch self {
        case .groupSnapshot(let count):
            return ["applied_group_count_bucket": Self.appliedGroupCountValue(count)]
        ...
        case .ruleGroupSnapshot(let payload): return payload.parameters
        case .strictLockStarted(let days, let payload),
             .strictLockExtended(let days, let payload):
            var parameters = payload.parameters
            parameters["strict_lock_days"] = days
            return parameters
        case .strictLockCompleted(let days):
            return ["strict_lock_days": days]
    ...
    private static func appliedGroupCountValue(_ count: Int) -> String {
        String(min(max(count, 0), ScreenTimeGroupPolicy.maxGroupCount))
    }
}
