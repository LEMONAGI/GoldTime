...
- **연장 불가 모드 표기 계약(2026-07-29 실사용 피드백 반영 — 되돌리지 말 것)**: ① 카드의 연장 불가
  ...
  상태)이고, 한 키로 합치면 이미 걸렸는지를 부제로만 판별하게 된다. 적용 전 제목은 **"모드"가
  아니라 "기간"**이다(2026-08-15 변경): 카드에서 하는 일은 기간(1/3/7일) 선택이고, "모드"는
  설정의 기능 토글(`settings.strictLock.title` "연장 불가 모드") 이름이라 둘이 같으면 이미 켠
  기능을 또 켜라는 뜻으로 읽힌다 — 되돌리지 말 것. ② 배지는 **"N일 남음"**
