...
- **`DayRule.isExplicitlyUnrestricted`(1.2.0)는 표시 전용 플래그** — 편집 화면에서 직접
  '제한 없음'을 골라 저장한 요일만 true(요일 그룹 행 생성 여부 결정). synthesized
  Equatable/Hashable에 포함되어 요일 그룹 행 분리가 값 동등성으로 성립하지만, 엔진·정책·
  ...
  false** — 깨지면 같은 파라미터의 제한 요일 그룹이 값 불일치로 두 행으로 쪼개진다.
...
- **Shield 분석 현재 이름**: DeviceActivity의 새 차단 시작은
  `SharedStore.enqueueShieldLockStarted(ruleMode:enforcementRule:)`로 `shield_lock_started`를 적재한다.
  `rule_mode`는 원본 그룹의 저장 방식(`uniform_*`/`weekday`), `enforcement_rule`은 그날 실제
  집행된 `daily`/`time_window`/`cooldown`이다. 설정 변경 도중의 동기 잠금은 사용 중
  막힘이 아니므로 적재하지 않는다. 구 `shield_hit`은 1.3.0 이전 이름이다.
...
- **휴식 중(`isInCooldown`) 쿨다운 그룹 편집 시 현재 휴식을 보존한다.** `syncDailyMonitoring`의 stale 정리는 변경된 쿨다운 그룹의 `cooldownTimer`까지 stop하면 안 된다 — `.keepCooldownRest`의 `registerCooldownGroup`이 휴식 중 early-return해서 타이머가 재등록되지 않아 휴식 종료(`handleCooldownTimerEnded`)가 영영 안 온다. stop 대상은 순수 함수 `cooldownActivitiesToStop`이 결정한다: 휴식 중이면 `cooldownUsage`만(타이머 보존), 그 외(삭제/규칙 전환/휴식 아님)는 usage+timer 둘 다. 변경된 설정은 휴식 종료 후 재충전 시점부터 적용된다(다음 사이클).
  **단 이 early-return은 "타이머가 이미 살아 있다"를 전제하고, 전체 stop 경로가 그 전제를 깬다** — 스크린타임 권한 철회→재승인, 그리고 설정의 "스크린 타임 재연결"(`reconnectMonitoring`의 `center.stopMonitoring()`)이 `cooldownTimer`까지 지우는데 휴식 상태(`cooldownUntil`)는 SharedStore에 남는다. 그러면 휴식 종료 콜백이 영영 오지 않아 **휴식이 자동으로 안 끝났다**(2026-07-29 실기기 실측: 30분이 지나도 안 풀리고 앱을 열어야 foreground 자가치유로 풀렸다). 그래서 `registerCooldownGroup`은 early-return 전에 `restoreCooldownTimerIfNeeded`로 **타이머가 실제로 살아 있는지 `center.activities`로 확인**하고, 없으면 남은 `cooldownEnd`로 재등록한다. 판정은 순수 함수 `CooldownMonitor.shouldRestoreCooldownTimer`: **살아 있으면 절대 재등록하지 않는다**(재등록은 intervalDidEnd를 새로 잡아 휴식을 리셋 — 위의 "편집이 휴식을 리셋하지 않는다" 계약이 깨진다), 남은 휴식이 `minimumScheduleSeconds`(15분, DeviceActivitySchedule 최소 간격)보다 짧으면 `intervalTooShort`로 실패하므로 시도하지 않는다(그 구간은 자가치유·자정 리셋 담당). 회귀 테스트 4건은 `CooldownTests`의 "사라진 휴식 타이머 복구" 절.
...
- **Analytics 대기 큐(`pendingAnalyticsEvents`)**: extension은 Firebase를 링크하지 않으므로(메인 앱 전용 `Core/Analytics/AnalyticsService`), extension에서 발생한 이벤트는 `SharedStore.enqueueShieldLockStarted`/`enqueueScreenTimeError`로 큐에 쌓고 메인 앱이 `AppLifecycleViewModel.appDidBecomeActive` → `drainPendingAnalyticsEvents()`에서 Firebase로 전송한다. `ScreenTimeManager`(Core)도 Firebase를 모르므로 override 등록 실패를 이 큐(`enqueueScreenTimeError`)로 보낸다. 페이로드는 `[String:String]`만 담는 plain Codable(`PendingAnalyticsEvent`)이라 extension 빌드에 Firebase 의존성을 들이지 않는다. 디코딩은 절대 throw하지 않고(실패 시 빈 배열), 상한 200건. **자정 리셋 대상 아님**(`clearAllShieldState`에 넣지 말 것 — 드레인 전 이벤트가 소실됨). 메인 앱 흐름 이벤트는 큐를 거치지 않고 `AnalyticsRepository`(Domain)로 직접 로깅한다. **단 `ShieldActionExtension`은 `SharedStore`를 링크하지 않아 같은 큐 키(`pendingAnalyticsEvents`)·동일 Codable 형태를 자체 복제(`PendingAnalyticsStore`)해 적재한다**(`shield_action_stop_selected`/`shield_action_extend_selected`, `ShieldActionExtension/CLAUDE.md` 참조). `SharedStore.enqueue*`를 직접 쓰는 extension은 DeviceActivityMonitor뿐이다.
...
- **알림 `interruptionLevel` 정책**: 지금 행동을 바꿔야 하는 알림만 `.timeSensitive`(집중 모드를
  뚫고 1시간 상단 고정) — 한도 임박 50·90%(`scheduleUsageAlert`), 재충전/다시 사용 가능
  (`scheduleRechargeAvailable`), 시간대 5분 전·종료(`scheduleTimeWindowAlert`), Shield "GoldTime
  가기"(`scheduleOpenAppNotification`·ShieldActionExtension). **아침 일일 요약
  (`scheduleDailyMorningNotification`)만 의도적으로 기본(`.active`)** — 어제 요약은 읽는 알림이지
  즉시 행동할 알림이 아니다(집중 모드를 뚫으면 성가심). 여기 새 알림을 추가할 땐 이 기준으로 판단.
  entitlement(`com.apple.developer.usernotifications.time-sensitive`)는 **메인 앱
  `GoldTime.entitlements`에만** 있으면 되고, extension이 발송하는 알림도 같은 앱 소속이라 커버된다
  (extension entitlements에 추가 불필요 — ShieldActionExtension이 이미 그렇게 동작 중). 권한도
  이미 `.timeSensitive` 옵션으로 요청하며, 기존 유저는 `timeSensitiveSetting == .disabled`일 때
  재요청한다(`requestAuthorizationIfNeeded`).
...
- **시간대 알림의 요일별 롤링 예약**: `NotificationService.rescheduleTimeWindowAlerts`는 **원본 그룹**(weekdayRules 보존)을 받아 비요일 그룹은 기존 index 기반 **매일 반복**을 유지하고, 요일별 그룹은 `resolved(on:now)`의 **오늘** 시간대만 날짜 포함(`year/month/day`) **일회성**으로 예약한다. 내일 `00:00~00:05` 시작 시간대의 5분 전은 이미 전날이므로 그 warn만 선예약한다. 자정 `handleHeartbeat`가 주 갱신 경로이고 규칙 저장·앱 활성화·BGTask의 `syncDailyMonitoring`이 보조한다. 현재 상한(5그룹×요일당 3시간대)에서 오늘 30개+자정 사전 경고 5개=35개라 pending 64를 넘지 않는다. 방어용 soft limit은 56개이고, 다른 GoldTime pending 수·8개 예약 여유를 빼고 가까운 시각부터 선택한다. **자정 00:00 종료 알림은 재예약이 경합할 수 있어 발화 전 1분 이내 pending 요청을 보존**한다(전날 23:59 시간대 종료 알림 소실 방지). 계획/IO는 `timeWindowAlertPlans`로 분리해 순수 테스트하며, `add` 완료 뒤 실제 pending id 수를 재확인해 시스템의 조용한 폐기를 `GTLog.timeWindow` 오류로 남긴다. 새 일회성 식별자는 날짜를 포함하므로 같은 그룹·시간대라도 날짜 간 충돌이 없다.
